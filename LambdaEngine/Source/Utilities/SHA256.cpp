...
namespace LambdaEngine
{
	...
	void SHA256::Transform(const byte* pMessage, uint32 block_nb)
	{
		uint32 w[64];
		uint32 wv[8];
		uint32 t1, t2;
	...
	void SHA256::Update(const byte* pMessage, uint32 len)
	{
		...
		memcpy(&m_block[m_len], pMessage, rem_len);
		...
		shifted_message = pMessage + rem_len;
		Transform(m_block, 1);
	...
	void SHA256::Final(byte* pDigest)
	{
		...
		for (i = 0; i < 8; i++) 
		{
			SHA2_UNPACK32(m_h[i], &pDigest[i << 2]);
		}
	}
