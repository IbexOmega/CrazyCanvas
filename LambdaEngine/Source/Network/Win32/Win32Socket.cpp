...
namespace LambdaEngine
{
	...
	{
		int socketType = protocol == PROTOCOL_TCP ? SOCK_STREAM : SOCK_DGRAM;
		int protocolType = protocol == PROTOCOL_TCP ? IPPROTO_TCP : IPPROTO_UDP;
		m_Socket = socket(AF_INET, socketType, protocolType);
	...
	bool Win32Socket::Receive(char* buffer, uint32 size, uint32& bytesReceived)
	{
