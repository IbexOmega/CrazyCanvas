...
namespace LambdaEngine
{
	...
	uint32 PacketManager2::EnqueuePacketReliable(NetworkPacket* pPacket, IPacketListener* pListener)
	{
	...
	uint32 PacketManager2::EnqueuePacketUnreliable(NetworkPacket* pPacket)
	{
		std::scoped_lock<SpinLock> lock(m_LockMessagesToSend);
		return EnqueuePacket(pPacket, 0);
	}

	uint32 PacketManager2::EnqueuePacket(NetworkPacket* pPacket, uint32 reliableUID)
	{
	...
	void PacketManager2::Flush(PacketTransceiver* pTransceiver)
	{
	...
	void PacketManager2::QueryBegin(PacketTransceiver* pTransceiver, std::vector<NetworkPacket*>& packetsReturned)
	{
	...
	void PacketManager2::QueryEnd(std::vector<NetworkPacket*>& packetsReceived)
	{
		m_PacketPool.FreePackets(packetsReceived);
	}

	void PacketManager2::Tick()
	{

	}

	PacketPool* PacketManager2::GetPacketPool()
	{
		return &m_PacketPool;
	}

	const NetworkStatistics* PacketManager2::GetStatistics() const
	{
		return &m_Statistics;
	}

	const IPEndPoint& PacketManager2::GetEndPoint() const
	{
		return m_IPEndPoint;
	}

	void PacketManager2::SetEndPoint(const IPEndPoint& ipEndPoint)
	{
		m_IPEndPoint = ipEndPoint;
	}

	void PacketManager2::Reset()
	{
	...
	void PacketManager2::FindPacketsToReturn(const std::vector<NetworkPacket*>& packetsReceived, std::vector<NetworkPacket*>& packetsReturned)
	{
	...
	void PacketManager2::UntangleReliablePackets(std::vector<NetworkPacket*>& packetsReturned)
	{
	...
	void PacketManager2::HandleAcks(const std::vector<uint32>& acks)
	{
	...
	void PacketManager2::GetReliableUIDsFromAcks(const std::vector<uint32>& acks, std::vector<uint32>& ackedReliableUIDs)
	{
	...
	void PacketManager2::GetReliableMessageInfosFromUIDs(const std::vector<uint32>& ackedReliableUIDs, std::vector<MessageInfo>& ackedReliableMessages)
	{
