...
layout(location = 1) out vec4 out_AO_Rough_Metal;
...
void main()
{
	...
	out_Albedo 				            = vec4(storedAlbedo, 0.0f);
	out_AO_Rough_Metal	                = vec4(storedAO, storedRoughness, storedMetallic, 0.0f);
    out_Compact_Normals                 = vec4(storedShadingNormal, storedGeometricNormal);
    out_Velocity                        = vec4(screenVelocity, 0.0f, 0.0f);
}
